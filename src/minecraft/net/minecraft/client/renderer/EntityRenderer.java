...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void renderWorld(float partialTicks, long finishTimeNano)
    {
        ...
        if (this.mc.gameSettings.anaglyph)
        {
            ...
            if(Cubitick.synctick) this.renderWorldPass(0, partialTicks, finishTimeNano);
            else this.renderWorldPass_Async(0, mc.timer.renderPartialTicks, mc.timerWorld.renderPartialTicks, finishTimeNano);
            anaglyphField = 1;
            GlStateManager.colorMask(true, false, false, false);
            if(Cubitick.synctick) this.renderWorldPass(1, partialTicks, finishTimeNano);
            else this.renderWorldPass_Async(1, mc.timer.renderPartialTicks, mc.timerWorld.renderPartialTicks, finishTimeNano);
            GlStateManager.colorMask(true, true, true, false);
        }
        else
        {
            if(Cubitick.synctick) this.renderWorldPass(2, partialTicks, finishTimeNano);
            else this.renderWorldPass_Async(2, mc.timer.renderPartialTicks, mc.timerWorld.renderPartialTicks, finishTimeNano);
        }
    ...
    private void renderWorldPass_Async(int pass, float ptick, float wtick, long finishTimeNano)
    {
        RenderGlobal renderglobal = this.mc.renderGlobal;
        ParticleManager particlemanager = this.mc.effectRenderer;
        boolean flag = this.isDrawBlockOutline();
        ...
        this.mc.mcProfiler.endStartSection("clear");
        GlStateManager.viewport(0, 0, this.mc.displayWidth, this.mc.displayHeight);
        this.updateFogColor(ptick);
        GlStateManager.clear(16640);
        this.mc.mcProfiler.endStartSection("camera");
        this.setupCameraTransform(ptick, pass);
        ActiveRenderInfo.updateRenderInfo(this.mc.thePlayer, this.mc.gameSettings.thirdPersonView == 2);
        this.mc.mcProfiler.endStartSection("frustum");
        ClippingHelperImpl.getInstance();
        this.mc.mcProfiler.endStartSection("culling");
        ICamera icamera = new Frustum();
        Entity entity = this.mc.getRenderViewEntity();
        double d0 = entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * (double)ptick;
        double d1 = entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * (double)ptick;
        double d2 = entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * (double)ptick;
        icamera.setPosition(d0, d1, d2);
        ...
        double ax = entity.posX - (entity.posX-lx)*(1.0-ptick)/(1.0-wtick);
        double ay = entity.posY - (entity.posY-ly)*(1.0-ptick)/(1.0-wtick);
        double az = entity.posZ - (entity.posZ-lz)*(1.0-ptick)/(1.0-wtick);
        
        if (this.mc.gameSettings.renderDistanceChunks >= 4)
        {
            this.setupFog(-1, ptick);
            this.mc.mcProfiler.endStartSection("sky");
            GlStateManager.matrixMode(5889);
            GlStateManager.loadIdentity();
            Project.gluPerspective(this.getFOVModifier(ptick, true), (float)this.mc.displayWidth / (float)this.mc.displayHeight, 0.05F, this.farPlaneDistance * 2.0F);
            GlStateManager.matrixMode(5888);
            renderglobal.renderSky(ptick, pass);
            GlStateManager.matrixMode(5889);
            GlStateManager.loadIdentity();
            Project.gluPerspective(this.getFOVModifier(ptick, true), (float)this.mc.displayWidth / (float)this.mc.displayHeight, 0.05F, this.farPlaneDistance * MathHelper.SQRT_2);
            GlStateManager.matrixMode(5888);
        }

        this.setupFog(0, ptick);
        GlStateManager.shadeModel(7425);

        if (entity.posY + (double)entity.getEyeHeight() < 128.0D)
        {
            this.renderCloudsCheck(renderglobal, ptick, pass);
        }
        ...
        RenderHelper.disableStandardItemLighting();
        this.mc.mcProfiler.endStartSection("terrain_setup");
        renderglobal.setupTerrain(entity, (double)ptick, icamera, this.frameCount++, this.mc.thePlayer.isSpectator());
        ...
        GlStateManager.pushMatrix();
        GlStateManager.disableAlpha();
        renderglobal.renderBlockLayer(BlockRenderLayer.SOLID, (double)ptick, pass, entity);
        GlStateManager.enableAlpha();
        renderglobal.renderBlockLayer(BlockRenderLayer.CUTOUT_MIPPED, (double)ptick, pass, entity);
        this.mc.getTextureManager().getTexture(TextureMap.LOCATION_BLOCKS_TEXTURE).setBlurMipmap(false, false);
        renderglobal.renderBlockLayer(BlockRenderLayer.CUTOUT, (double)ptick, pass, entity);
        this.mc.getTextureManager().getTexture(TextureMap.LOCATION_BLOCKS_TEXTURE).restoreLastBlurMipmap();
        GlStateManager.shadeModel(7424);
        GlStateManager.alphaFunc(516, 0.1F);
        ...
        if (!this.debugView)
        {
            ...
            // Cubitick
            entity.lastTickPosX = ax;
            entity.lastTickPosY = ay;
            entity.lastTickPosZ = az;
            
            renderglobal.renderEntities(entity, icamera, wtick);
        ...
        if (flag && this.mc.objectMouseOver != null && !entity.isInsideOfMaterial(Material.WATER))
        {
            ...
            GlStateManager.disableAlpha();
            this.mc.mcProfiler.endStartSection("outline");
            renderglobal.drawSelectionBox(entityplayer, this.mc.objectMouseOver, 0, ptick);
            GlStateManager.enableAlpha();
        }

        if (this.mc.debugRenderer.func_190074_a())
        {
            this.mc.debugRenderer.func_190073_a(ptick, finishTimeNano);
        }
        ...
        GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
        this.mc.getTextureManager().getTexture(TextureMap.LOCATION_BLOCKS_TEXTURE).setBlurMipmap(false, false);
        renderglobal.drawBlockDamageTexture(Tessellator.getInstance(), Tessellator.getInstance().getBuffer(), entity, ptick);
        this.mc.getTextureManager().getTexture(TextureMap.LOCATION_BLOCKS_TEXTURE).restoreLastBlurMipmap();
        GlStateManager.disableBlend();
        ...
        if (!this.debugView)
        {
            ...
            RenderHelper.disableStandardItemLighting();
            this.setupFog(0, wtick);
            this.mc.mcProfiler.endStartSection("particles");
            particlemanager.renderParticles(entity, wtick);
            this.disableLightmap();
        }
        ...
        GlStateManager.enableCull();
        this.mc.mcProfiler.endStartSection("weather");
        this.renderRainSnow(wtick);
        GlStateManager.depthMask(true);
        renderglobal.renderWorldBorder(entity, wtick);
        GlStateManager.disableBlend();
        GlStateManager.enableCull();
        GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
        GlStateManager.alphaFunc(516, 0.1F);
        ...
        this.mc.mcProfiler.endStartSection("translucent");
        renderglobal.renderBlockLayer(BlockRenderLayer.TRANSLUCENT, (double)ptick, pass, entity);
        GlStateManager.shadeModel(7424);
        GlStateManager.depthMask(true);
        GlStateManager.enableCull();
        GlStateManager.disableBlend();
